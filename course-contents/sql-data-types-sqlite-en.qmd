...
CREATE TABLE StrictTyping (
	DynamicTypingId INTEGER PRIMARY KEY AUTOINCREMENT,
	ColumnNumber INTEGER) strict;
```

Then try to insert text value to integer column.

```sql
INSERT INTO StrictTyping (ColumnNumber) VALUES('Atilla');
```

you will get following error.

```
SQL Error [19]: [SQLITE_CONSTRAINT_DATATYPE] An insert or update attempted to store a value inconsistent with the column's declared type in a table defined as STRICT (cannot store TEXT value in INTEGER column StrictTyping.ColumnNumber)
```
